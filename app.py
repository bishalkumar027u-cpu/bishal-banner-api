...
async def home():
    return {"message": "⚡ Ultra Fast Banner API Running",
           ...
           "Api Endpoint": "/profile?uid={uid}&region={region}",
...
async def get_banner(uid: str, region: str = "IND"):
    ...
    try:
        resp = await client.get(f"{INFO_API_URL}?region={region}&uid={uid}")
        ...
        if pin_id and str(pin_id) != "0":
            ...
            pin_task = asyncio.sleep(0)
            pin_id = None

        results = await asyncio.gather(avatar_task, banner_task, pin_task if pin_id else asyncio.sleep(0))
        avatar_bytes, banner_bytes = results[0], results[1]
        pin_bytes = results[2] if pin_id else b''
